...
const TempBlockScreen: React.FC<{ unblockAt: number }> = ({ unblockAt }) => {
  const [secsLeft, setSecsLeft] = React.useState(() => Math.max(0, Math.floor((unblockAt - Date.now()) / 1000)));

  React.useEffect(() => {
    const tick = () => setSecsLeft(Math.max(0, Math.floor((unblockAt - Date.now()) / 1000)));
    tick();
  ...
  return (
    <div className="min-h-screen bg-neutral-950 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-neutral-900 border border-amber-500/20 p-8 rounded-[40px] text-center shadow-2xl">
        ...
        <div className="bg-neutral-950 border border-amber-500/20 rounded-2xl px-6 py-5 mb-4">
          <p className="text-[10px] font-black uppercase tracking-widest text-amber-500/60 mb-2">Time remaining</p>
          <p className="text-5xl font-black font-mono text-amber-400 tracking-widest tabular-nums">{fmt}</p>
        </div>
        <p className="text-neutral-600 text-xs">The page will automatically unlock when this timer reaches zero.</p>
      </div>
    </div>
  );
};
...
const AppContent: React.FC = () => {
  ...
  if (userData?.isBlocked) {
    ...
    return isTempBlock
      ? <TempBlockScreen unblockAt={userData.tempBlockedUntil!} />
      : (
        <div className="min-h-screen bg-neutral-950 flex items-center justify-center p-4">
